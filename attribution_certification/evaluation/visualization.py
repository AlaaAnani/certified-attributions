...
def visualize_examples(attributions, images, num_examples=10, head_idx=0, head_pos_idx=0, img_dims=(224, 224), scale=2, 
                       percentile=0.5, display_negative=False, plt_fig_axis=None, fig_size=2, bin_positions_list=[0.0, 0.02, 0.05, 0.5, 0.95, 0.98, 1.0]):
    """
    ...
    aggatt_bin_dict = {i: [] for i in range(num_bins)}
    for aggatt_bin in range(num_bins):
        bin_start_idx = int(bin_positions_list[aggatt_bin] * num_attributions)
        bin_end_idx = int(
            bin_positions_list[aggatt_bin + 1] * num_attributions)
        ...
        for example_idx in range(num_examples):
            ...
            aggatt_bin_dict[aggatt_bin].append(images[sorted_indices][attribution_idx].permute(1, 2, 0).cpu().numpy(), 
                                               display_attributions[attribution_idx, head_idx].permute(1, 2, 0).cpu().numpy(),
                                               _get_bounding_box(head_pos_idx, img_dims=img_dims, scale=scale))
    ...
    return plt_fig, aggatt_bin_dict
...
def _get_bounding_box(head_pos_idx, img_dims=(224, 224), scale=2, linewidth=3, edgecolor='g'):
    """
    ...
    box = matplotlib.patches.Rectangle(
        (box_x, box_y), img_dims[0] - 0.25, img_dims[1] - 0.25, edgecolor=edgecolor, linewidth=linewidth, facecolor='none')
    return box
